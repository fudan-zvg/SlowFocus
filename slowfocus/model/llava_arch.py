...
from slowfocus.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN


def load_state_dict_maybe_zero_3(model, state_dict, strict=False, ignore_status=False):
    def check_zero_3_optimization(model):
        for name, param in model.named_parameters():
            if hasattr(param, 'ds_id'):
                return True

    if check_zero_3_optimization(model):
        import deepspeed
        ...
        def load(module, local_state_dict, prefix=""):
            local_metadata = {}
            with zero.GatheredParameters(list(module.parameters(recurse=False)), modifier_rank=0):
...
class LLaVAMetaModel:
    ...
    def initialize_attention_modules(self, model_args, for_eval=False, non_lora_dict=None):
        ...
        self.config.temporal_embedding = getattr(model_args, "temporal_embedding", None)
        self.config.lfreq_alignment = getattr(model_args, "lfreq_alignment", None)
        self.config.mix_attn = getattr(model_args, "mix_attn", None)
        ...
        if self.config.mix_attn:
            if 'v1' in self.config.mix_attn or 'bi' in self.config.mix_attn:
                self.mix_attn_hfreq_projector = torch.nn.Linear(self.config.mm_hidden_size, self.config.mm_hidden_size)
                self.mix_attn_lfreq_projector = torch.nn.Linear(self.config.mm_hidden_size, self.config.mm_hidden_size)
            else:
                self.mix_attn_q_projector = torch.nn.Linear(self.config.mm_hidden_size,
                                                            self.config.mm_hidden_size)
                self.mix_attn_k_projector = torch.nn.Linear(self.config.mm_hidden_size,
                                                            self.config.mm_hidden_size)
                self.mix_attn_v_projector = torch.nn.Linear(self.config.mm_hidden_size,
                                                            self.config.mm_hidden_size)

        if self.config.temporal_embedding == 'v1':
            self.temporal_embedding = torch.nn.Embedding(1000, 1024)
        ...
        if 'freeze_all' in self.config.bert_type:
            ...
            if self.config.temporal_embedding:
                self.temporal_embedding.requires_grad_(False)
        ...
        if pretrain_mm_mlp_adapter is not None:
        ...
        elif non_lora_dict is not None:
            att_projector_weights = non_lora_dict
            if 'freeze' in self.config.bert_type:
                if 'freeze_all' in self.config.bert_type:
                    print("All qformer weights are not contained in non_lora_dict, loading from base model...")
                    trainable_module = ['vlm_att_encoder', 'vlm_att_ln', 'vlm_att_query', 'vlm_att_projector',
                                        'vlm_att_key_projector',
                                        'vlm_att_val_projector', 'temporal_embedding', 'mix_attn_q_projector',
                                        'mix_attn_k_projector', 'mix_attn_v_projector', 'mix_attn_hfreq_projector',
                                        'mix_attn_lfreq_projector']
                else:
                    print("Pretrained qformer weights are not contained in non_lora_dict, loading from base model...")
                    trainable_module = ['vlm_att_encoder', 'vlm_att_ln', 'vlm_att_query']
            ...
            trainable_module = ['vlm_att_encoder', 'vlm_att_projector', 'vlm_att_key_projector', 
                                'vlm_att_val_projector', 'temporal_embedding', 'vlm_att_query',
                                'vlm_att_visual_proj', 'vlm_att_ln', 'mix_attn_q_projector', 'mix_attn_k_projector',
                                'mix_attn_v_projector', 'mix_attn_hfreq_projector', 'mix_attn_lfreq_projector']
            if hasattr(model_args, 'model_name_or_path'):
                model_save_path = model_args.model_name_or_path
            else:
                model_save_path = model_args.model_path
        ...
        load_state_dict_maybe_zero_3(self.vlm_att_encoder, bert_dict)
        load_state_dict_maybe_zero_3(self.vlm_att_projector, get_w(att_projector_weights, 'vlm_att_projector'))
        load_state_dict_maybe_zero_3(self.vlm_att_key_projector, get_w(att_projector_weights, 'vlm_att_key_projector'))
        load_state_dict_maybe_zero_3(self.vlm_att_val_projector, get_w(att_projector_weights, 'vlm_att_val_projector'))
        if self.config.temporal_embedding:
            load_state_dict_maybe_zero_3(self.temporal_embedding, get_w(att_projector_weights, 'temporal_embedding'))
        if self.config.mix_attn and for_eval:
            if 'v1' in self.config.mix_attn or 'bi' in self.config.mix_attn:
                load_state_dict_maybe_zero_3(self.mix_attn_hfreq_projector,
                                             get_w(att_projector_weights, 'mix_attn_hfreq_projector'))
                load_state_dict_maybe_zero_3(self.mix_attn_lfreq_projector,
                                             get_w(att_projector_weights, 'mix_attn_lfreq_projector'))
            else:
                load_state_dict_maybe_zero_3(self.mix_attn_q_projector, get_w(att_projector_weights, 'mix_attn_q_projector'))
                load_state_dict_maybe_zero_3(self.mix_attn_k_projector, get_w(att_projector_weights, 'mix_attn_k_projector'))
                load_state_dict_maybe_zero_3(self.mix_attn_v_projector, get_w(att_projector_weights, 'mix_attn_v_projector'))
        ...
        if "qformer" in self.config.bert_type:
            ...
            if "pretrain" in self.config.bert_type:
                ...
                load_state_dict_maybe_zero_3(self.vlm_att_ln, get_w(att_projector_weights, 'vlm_att_ln'))

        if self.vlm_att_bert_proj is not None:
            print('Loading vlm_att_bert_proj weights...')
            load_state_dict_maybe_zero_3(self.vlm_att_bert_proj, get_w(att_projector_weights, 'vlm_att_bert_proj'))
        ...
        if for_eval:
            ...
            if self.config.temporal_embedding:
                self.temporal_embedding = self.temporal_embedding.to(device=device_type, dtype=weight_type)
            if self.config.mix_attn:
                if 'v1' in self.config.mix_attn or 'bi' in self.config.mix_attn:
                    self.mix_attn_hfreq_projector = self.mix_attn_hfreq_projector.to(device=device_type, dtype=weight_type)
                    self.mix_attn_lfreq_projector = self.mix_attn_lfreq_projector.to(device=device_type, dtype=weight_type)
                else:
                    self.mix_attn_q_projector = self.mix_attn_q_projector.to(device=device_type, dtype=weight_type)
                    self.mix_attn_k_projector = self.mix_attn_k_projector.to(device=device_type, dtype=weight_type)
                    self.mix_attn_v_projector = self.mix_attn_v_projector.to(device=device_type, dtype=weight_type)
...
class LLaVAMetaForCausalLM(ABC):
    ...
    def encode_images(self, images, prompts=None, temporal_prompts=None, lfreq_token_ids=None, hfreq_token_ids=None, image_counts=None, long_video=False):
        if long_video:
        ...
        image_features = self.vlm_attention(image_features,
                                            prompts=prompts,
                                            temporal_prompts=temporal_prompts,
    ...
    def vlm_attention(self, image_features, prompts=None, temporal_prompts=None, lfreq_token_ids=None, hfreq_token_ids=None, image_counts=None, long_video=False):
        img_feat_lst = []
        if image_counts is None:
            assert len(image_features) == len(prompts), f"Size mismatch! image_features: {len(image_features)}, prompts: {len(prompts)}"
            assert len(temporal_prompts) == len(image_features), f"Size mismatch! temporal prompts: {len(temporal_prompts)}, image_features: {len(image_features)}"
        else:
            assert len(prompts) == len(image_counts), f"Size mismatch! prompts: {len(prompts)}, image_counts: {len(image_counts)}"
            assert len(temporal_prompts) == len(image_counts), f"Size mismatch! temporal prompts: {len(temporal_prompts)}, image_counts: {len(image_counts)}"
        ...
        for _idx in range(len(prompts)):
            ...
            if image_counts is None:
                ...
                temporal_prompt = temporal_prompts[_idx].expand(len(prompts[_idx]))
                lfreq_token_ids = lfreq_token_ids[_idx].expand(len(prompts[_idx]))
                hfreq_token_ids = hfreq_token_ids[_idx].expand(len(prompts[_idx]))
                ...
                temporal_prompt = temporal_prompts[_idx]
                temporal_prompt = temporal_prompt[None].expand(len(prompts[_idx]), -1).flatten(0,1)
                lfreq_token_id = lfreq_token_ids[_idx]
                lfreq_token_id = lfreq_token_id[None].expand(len(prompts[_idx]), -1).flatten(0,1)
                hfreq_token_id = hfreq_token_ids[_idx]
                hfreq_token_id = hfreq_token_id[None].expand(len(prompts[_idx]), -1).flatten(0,1)
            ...
            mm_output = self.mmout_generation(input_ids, img_att_prompt, attention_masks, bert_feat, long_video)
            
            text_q = self.get_model().vlm_att_projector(mm_output)
            final_token = self.token_generation(text_q, img_feat_prompt, temporal_prompt, lfreq_token_id, hfreq_token_id, long_video=long_video,
                                                image_count=image_counts[_idx] if image_counts is not None else 1)
    ...
    def mmout_generation(self, input_ids, img_att_prompt, attention_masks, bert_feat, long_video):
        if "qformer" in self.config.bert_type:
    ...
    def token_generation(self, text_q, vis_embed, temporal_prompt, lfreq_token_id, hfreq_token_id, long_video=False, image_count=1):
        ctx_embed = self.get_model().vlm_att_key_projector(vis_embed)
        # Key part 1: calculate context-related embedding
        if self.config.lfreq_alignment == 'cross_attn':
            len_prompt = int(ctx_embed.shape[0] / image_count)
            N_t, C = ctx_embed.shape[1:]
            lfreq_embed = ctx_embed.reshape(len_prompt, image_count, N_t, C).mean(2)
            hfreq_embed = text_q.reshape(len_prompt, -1, C)

            weight = lfreq_embed @ hfreq_embed.transpose(-1, -2)
            weight = weight / (lfreq_embed.shape[-1] ** 0.5)
            ctx_embed = (weight.softmax(-1) @ hfreq_embed).reshape(-1, C)
        else:
        ...
        # temporal encoding
        if self.config.temporal_embedding == 'v1':
            temporal_prompt = temporal_prompt * 1000
            temporal_prompt = torch.clamp(temporal_prompt.long(), min=0, max=999)
            temporal_prompt = self.get_model().temporal_embedding(temporal_prompt)
            temporal_prompt = temporal_prompt.unsqueeze(-1).repeat(1, 1, vis_embed.shape[1]).permute(0, 2, 1).contiguous()
            vis_embed += temporal_prompt

        if self.config.mix_attn == 'bi':
            lfreq_token = vis_embed[lfreq_token_id].flatten(0, 1)
            hfreq_token = vis_embed[hfreq_token_id].flatten(0, 1)
            l_proj_token = self.get_model().mix_attn_lfreq_projector(lfreq_token)
            h_proj_token = self.get_model().mix_attn_hfreq_projector(hfreq_token)
            weight1 = h_proj_token @ l_proj_token.transpose(-1, -2)
            weight1 = weight1 / (l_proj_token.shape[-1] ** 0.5)
            h_mix_token = weight1.softmax(-1) @ lfreq_token
            weight2 = l_proj_token @ h_proj_token.transpose(-1, -2)
            weight2 - weight2 / (h_proj_token.shape[-1] ** 0.5)
            l_mix_token = weight2.softmax(-1) @ hfreq_token
            mix_embed = torch.zeros_like(vis_embed).to(vis_embed)
            mix_embed[hfreq_token_id] = h_mix_token.reshape(-1, vis_embed.shape[1], self.config.mm_hidden_size)
            mix_embed[lfreq_token_id] = l_mix_token.reshape(-1, vis_embed.shape[1], self.config.mm_hidden_size)
            vis_embed = vis_embed + mix_embed
    ...
    def prepare_inputs_labels_for_multimodal(
        self, input_ids, attention_mask, past_key_values, labels, images, temporal_pos, lfreq_token_ids, hfreq_token_ids, prompts=None
    ): 
        if prompts is None and hasattr(self, 'prompts'):
            prompts = self.prompts

        assert temporal_pos is not None
        temporal_prompts = temporal_pos
        ...
        if type(images) is list or images.ndim == 5:
            ...
            image_features = self.encode_images(concat_images, prompts, temporal_prompts, lfreq_token_ids, hfreq_token_ids, image_counts, long_video=long_video)
        else:
            image_features = self.encode_images(images, prompts, temporal_prompts, lfreq_token_ids, hfreq_token_ids, long_video=long_video)
